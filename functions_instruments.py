...
def generate_record_raw(df):
    cols_remove = []
    cols_numeric = ['단가', '번역료', '수령액', 'RT', 'RTM', 'CPE', 'CPER', '보정환율2']
    cols_date = ['마감일', '대금 수령일']
    cols_use = ['발주처', '방영 채널', '작품명 / 시즌', 'Ep', 'RT', 'RTM', 'CPE', 'CPER', '보정환율2', 
                '영상 종류', '작업 종류', '마감일',
    ...
    tdf = tdf.assign(
        new_unit_price = np.where(tdf['단가']!=tdf['단가'], tdf['보정환율2']*(tdf['RTM'] + np.nan_to_num(tdf['CPE']*tdf['CPER'])/tdf['RT']), tdf['단가'])
        )
    #
    return tdf[cols_use]
